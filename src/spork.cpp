...
// Encocoin: on startup load spork values from previous session if they exist in the sporkDB
...
int64_t GetSporkValue(int nSporkID)
{
    ...
    if (mapSporksActive.count(nSporkID)) {
        ...
        if (nSporkID == SPORK_17_STAKE_REQ_SZ_MODE) r = SPORK_17_STAKE_REQ_SZ_MODE_DEFAULT;
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if (strName == "SPORK_17_STAKE_REQ_SZ_MODE") return SPORK_17_STAKE_REQ_SZ_MODE;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if (id == SPORK_17_STAKE_REQ_SZ_MODE) return "SPORK_17_STAKE_REQ_SZ_MODE";
