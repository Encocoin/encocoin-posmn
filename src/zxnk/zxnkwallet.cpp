...
CzXNKWallet::CzXNKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zxnk seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZXNKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZXNKSeed_deprecated()) {
                    LogPrintf("%s: Updated zXNK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zxnk seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zxnk wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzXNKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZXNKCount(nCountLastUsed);
    else if (!walletdb.ReadZXNKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzXNKWallet::Lock()
{
    seedMaster = 0;
}

void CzXNKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzXNKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzXNKWallet::LoadMintPoolFromDB()
{
...
void CzXNKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzXNKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzXNKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zxnkTracker->HasPubcoinHash(pMint.first)) {
...
bool CzXNKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZXNK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zxnkTracker which also adds to database
    pwalletMain->zxnkTracker->Add(dMint, true);
...
uint512 CzXNKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzXNKWallet::GenerateDeterministicZXNK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzXNKWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzXNKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzXNKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
