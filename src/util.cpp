...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "encocoin" is a composite category enabling all Encocoin-related debug output
            if (ptrCategory->count(std::string("encocoin"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Encocoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Encocoin
// Mac: ~/Library/Application Support/Encocoin
// Unix: ~/.encocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Encocoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "encocoin.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty encocoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override encocoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "encocoind.pid"));
