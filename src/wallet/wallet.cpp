        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if ((GetBoolArg("-zxnkstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = zxnkTracker->ListMints(true, true, fUpdate);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 XNK collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 XNK collateral
...
std::string CWallet::GetUniqueWalletBackupName(bool fzxnkAuto) const
{
...
void CWallet::ZPivBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozxnkbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zxnkbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zxnkbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no zXNK.
    /*
